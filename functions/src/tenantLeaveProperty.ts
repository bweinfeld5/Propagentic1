 ...
 */
export const tenantLeaveProperty = onCall(async (request: CallableRequest<{ 
  propertyId: string; 
  reason?: string;
  moveOutDate?: string;
}>) => {
  ...
  try {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (!tenantProfileSnap.exists) {
        throw new HttpsError("not-found", "Tenant profile not found.");
      }
      ...
      if (!tenantProperties.includes(propertyId)) {
        throw new HttpsError(
          "permission-denied",
          "You are not associated with this property."
        );
      }
      ...
      const updatedTenantProperties = tenantProperties.filter((id: string) => id !== propertyId);
      ...
      const updatedPropertyTenants = propertyTenants.filter((id: string) => id !== tenantUid);
      ...
      if (landlordProfileSnap.exists) {
        ...
        const updatedAcceptedTenants = acceptedTenants.filter((id: string) => id !== tenantUid);
        ...
        const updatedAcceptedTenantDetails = acceptedTenantDetails.filter(
          (record: any) => record.tenantId !== tenantUid || record.propertyId !== propertyId
        );
    ...
    return {
      success: true,
      message: "Successfully left the property.",
      ...result
    };
    ...
    if (error instanceof HttpsError) {
      ...
      throw new HttpsError("internal", "An unexpected error occurred while processing the departure.");
    }
  }
}); 