...
class JobService {
  ...
  /**
   * Contractor accepts a job bid. Validates ownership, updates job and bid, rejects competing bids.
   */
  async acceptJobByContractor(contractorId: string, bidId: string, jobId: string): Promise<void> {
    try {
      ...
      const bidData = bidDoc.data() as Bid;
      if (bidData.contractorId !== contractorId) throw new Error('Unauthorized: Bid does not belong to contractor');
      ...
      const otherBidsSnapshot = await getDocs(otherBidsQuery);
      otherBidsSnapshot.docs.forEach(docSnap => {
        if (docSnap.id !== bidId) {
      ...
      console.error('Error in acceptJobByContractor:', error);
      throw error;
    }
  }

  /**
   * Contractor rejects their own bid for a job. Validates ownership.
   */
  async rejectJobByContractor(contractorId: string, bidId: string, jobId: string): Promise<void> {
    try {
      // Validate bid ownership
      const bidRef = doc(this.bidsRef, bidId);
      const bidDoc = await getDoc(bidRef);
      ...
      console.error('Error in rejectJobByContractor:', error);
      throw error;
    }
  }
