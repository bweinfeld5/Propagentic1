...
export const createEscrowPayment = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (enableMilestones && milestones.length > 0) {
      ...
      processedMilestones = milestones.map((milestone: any, index: number) => {
...
export const confirmEscrowPayment = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const releaseEscrowFunds = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    if (milestoneId && escrowData.milestones) {
      const milestone = escrowData.milestones.find((m: any) => m.id === milestoneId);
      if (!milestone) {
    ...
    if (milestoneId && escrowData.milestones) {
      ...
      const updatedMilestones = escrowData.milestones.map((m: any) => {
        if (m.id === milestoneId) {
          return {
            ...m,
            status: 'released',
      ...
      const allReleased = updatedMilestones.every((m: any) => m.status === 'released');
      if (allReleased) {
...
export const refundEscrowFunds = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const getEscrowAccount = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const autoReleaseEscrowFunds = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      for (const escrowDoc of escrowSnapshot.docs) {
        ...
        if (!holdStartDate || !autoReleaseAfterDays) continue;
        ...
        if (releaseDate <= now.toDate()) {
          ...
          try {
            ...
            const contractorData = contractorDoc.data()!;
            if (!contractorData.stripeAccountId) continue;
            ...
            await escrowDoc.ref.update({
              ...
              metadata: {
                ...escrowData.metadata,
                autoReleaseError: transferError.message,
                autoReleaseAttemptedAt: admin.firestore.FieldValue.serverTimestamp()
              }
            });
          }
        }
